...
class TestCaptchaSolver:
    ...
    def run(self):
        ...
        game_image = self.get_game_window_image()

        x, y, w, h = self.get_captcha_window_contour(game_image)      

        captcha_image = game_image[y:y+h,x:x+w]
        ...
        for x in range(10):                      
            ...
            for i in range(0, len(digits_to_validate)):
                ...
                while retry:                    
                    ...
                    template = cv2.imread(config['TEMPLATES'][digits_to_validate[i][0]])
