...
class GameStatusWatcherSelenium:
    ...
    def __connect_wallet_and_start(self, reconnect=False):
        ...
        self.__image_helper.wait_until_match_is_found(self.__selenium_helper.selenium_screenshot_to_opencv, [
                                                      self.webdriver], self.__config['TEMPLATES']['map_screen_validator'], 2, 0.05, True)
    ...
    def __verify_and_handle_game_error(self):
        error = self.__image_helper.wait_until_match_is_found(self.__selenium_helper.selenium_screenshot_to_opencv, [
                                                              self.webdriver], self.__config['TEMPLATES']['error_message'], 2, 0.05)
        if error:
            logging.error('Error on game, refreshing page.')
            self.webdriver.refresh()
            self.__connect_wallet_and_start(True)

        expected_screen = self.__image_helper.wait_until_match_is_found(self.__selenium_helper.selenium_screenshot_to_opencv, [
                                                                        self.webdriver], self.__config['TEMPLATES']['map_screen_validator'], 2, 0.05)
    ...
    def __verify_and_handle_newmap(self):
        newmap = self.__image_helper.wait_until_match_is_found(self.__selenium_helper.selenium_screenshot_to_opencv, [
                                                               self.webdriver], self.__config['TEMPLATES']['newmap_button'], 2, 0.05)
        if newmap:
    ...
    def __verify_and_handle_heroes_status(self):
        ...
        if(self.__image_helper.wait_until_match_is_found(self.__selenium_helper.selenium_screenshot_to_opencv, [self.webdriver], self.__config['TEMPLATES']['work_active_button'], timeout, 0.02)
           or self.__image_helper.wait_until_match_is_found(self.__selenium_helper.selenium_screenshot_to_opencv, [self.webdriver], self.__config['TEMPLATES']['work_button'], timeout, 0.02)):
            logging.warning('Starting __click_all_work_buttons')
    ...
    def __click_all_work_buttons(self):
        ...
        logging.warning('inactive_work_button_list count:' +
        ...
        while len(inactive_work_button_list) == 0 and count <= 5:
            ...
            logging.warning('First while inactive_work_button_list count:' +
                            len(inactive_work_button_list))
            logging.warning('First while active_work_button_list count:' +
            ...
            if active_work_button_list:
                ...
                self.__windows_action_helper.click_and_scroll_down(
                    last_active_button_x, last_active_button_y)
                inactive_work_button_list = self.__image_helper.wait_all_until_match_is_found(self.__selenium_helper.selenium_screenshot_to_opencv, [
        ...
        count = 0
        while len(inactive_work_button_list) > 0 and count <= 5:
            logging.warning('Second while inactive_work_button_list count BEFORE clicking:' +
                            len(inactive_work_button_list))
            for (x, y) in inactive_work_button_list:
                self.__selenium_helper.click_element_by_position(
                    self.webdriver, x, y)
                time.sleep(0.5)
            last_button_x, last_button_y = inactive_work_button_list[len(
                inactive_work_button_list)-1]
            self.__windows_action_helper.click_and_scroll_down(
                last_button_x, last_button_y)
            inactive_work_button_list = self.__image_helper.wait_all_until_match_is_found(self.__selenium_helper.selenium_screenshot_to_opencv, [
                                                                                          self.webdriver], self.__config['TEMPLATES']['work_button'], 25, 0.02, False, False)
            logging.warning('Second while inactive_work_button_list count AFTER clicking:' +
                            len(inactive_work_button_list))
...
# region Util
    ...
    def __find_and_click_by_template(self, template_path, confidence_level=0.1):
        result_match = self.__image_helper.wait_until_match_is_found(self.__selenium_helper.selenium_screenshot_to_opencv, [
                                                                     self.webdriver], template_path, self.__config['TIMEOUT'].getint('imagematching'), confidence_level)

        if result_match:
            self.__selenium_helper.click_element_by_position(
                self.webdriver, result_match.x, result_match.y)

    def __find_all_and_click_by_template(self, template_path):
        result_match = self.__image_helper.wait_all_until_match_is_found(self.__selenium_helper.selenium_screenshot_to_opencv, [
                                                                         self.webdriver], template_path, self.__config['TIMEOUT'].getint('imagematching'))
