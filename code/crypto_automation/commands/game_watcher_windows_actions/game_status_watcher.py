...
class GameStatusWatcherActions:
    ...
    def start_game(self):
        ...
        connection_error_handling = Thread(
            self.__thread_safe, self.__validate_game_connection, self.__config['RETRY'].getint('verify_zero_coins'))
    ...
    def __click_all_work_buttons(self):
        ...
        ], self.__config['TEMPLATES']['work_button'], 2, 0.05, should_grayscale=False)
        ...
        while len(result_match) == 0 and count <= 5:
            ...
            ], self.__config['TEMPLATES']['work_active_button'], 25, 0.02, should_grayscale=False)
